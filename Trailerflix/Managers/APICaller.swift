...
class Constants {
    static let API_KEY = "6c615b42ccc046bfe6569e98a358c328"
    static let BASE_URL = "https://api.themoviedb.org/3/"
    static let YOUTUBE_API_KEY = "AIzaSyB9TyiL_Kma5jyhYC2wCUMt68G57baOuFU"
...
class APICaller {
    ...
    func getDiscoverMovie(completion: @escaping (Result<[Title], Error>) -> Void) {
        guard let url = URL(string: "\(Constants.BASE_URL)discover/movie?api_key=\(Constants.API_KEY)&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_watch_monetization_types=flatrate") else {return}
       let task = URLSession.shared.dataTask(with: URLRequest(url: url)) { data, _, error in
           guard let data = data, error == nil else {
               return
           }
    ...
    func search(with query: String, completion: @escaping (Result<[Title], Error>) -> Void) {
        ...
        guard let url = URL(string: "\(Constants.BASE_URL)search/movie?api_key=\(Constants.API_KEY)&query=\(query)") else {return}
    ...
    func getMovie(with query: String, completion: @escaping (Result<VideoElement, Error>) -> Void) {
        guard let query = query.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) else {return}
        guard let url = URL(string: "\(Constants.YOUTUBE_BASE_URL)search?key=\(Constants.YOUTUBE_API_KEY)&q=\(query)") else {return}
